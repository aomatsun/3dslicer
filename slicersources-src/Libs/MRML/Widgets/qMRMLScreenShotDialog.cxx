...
void qMRMLScreenShotDialog::saveAs()
{
  ...
  QSettings settings;
  QString dirpath = settings.value("ExportDirectory").toString();
  QDate datetime = QDate::currentDate();
  QString strnow = datetime.toString(Qt::ISODate);
  strnow = strnow.replace("-", "");
  int i = 0;
  QString savePath = dirpath + "/"+ "Session_" + strnow +"_"+ QString::number(i) + ".jpg" ;
  while (QFile::exists(savePath))
  {
      i++;
      savePath = dirpath + "/" + "Session_" + strnow + "_" + QString::number(i) + ".jpg";
  }
  if (savePath != "")
    {
    QImage qimage;
    ...
    QMessageBox msgBox;
    msgBox.setText("保存されました");
    msgBox.exec();
    }
  //qMRMLScreenShotDialog::close();
}
