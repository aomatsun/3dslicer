...
//  this->BackgroundOpacitySlider->popup()->setAlignment(Qt::AlignBottom | Qt::AlignLeft);
 ...
 this->connect(this->SliceOrientationSelector, SIGNAL(currentIndexChanged(QString)),
  ...
  QObject::connect(this->actionReset_to_volume_plane, SIGNAL(triggered()),
                   q, SLOT(resetSliceToBackground()));
...
//  this->connect(this->BackgroundComboBox, SIGNAL(nodeActivated(vtkMRMLNode*)),
 ...
 // this->SliceCompositeButton->setMenu(this->CompositingMenu);
  ...
  this->SliceResetToVolumePlaneButton->setDefaultAction(
    this->actionReset_to_volume_plane);

  this->SliceMoreOptionButton->setVisible(true);
  ...
  this->ShowReformatWidgetToolButton->hide();
  this->SliceCompositeButton->hide();
  this->SliceSpacingButton->hide();
  this->SliceMoreOptionButton->hide();
  this->SliceOrientationSelector->hide();
  this->LightBoxToolButton->hide();
  this->OrientationMarkerButton->hide();
  this->MoreButton->hide();

  QIcon fillIcon(":/Icons/ResetVolume.png");
  this->SliceResetToVolumePlaneButton->setIcon(fillIcon);
}
...
void qMRMLSliceControllerWidgetPrivate::init()
{
  ...
  this->FitToWindowToolButton->setFixedSize(20, 20);
  ...
  this->LightBoxToolButton->hide();
  this->OrientationMarkerButton->hide();
  this->LabelMapComboBox->hide();
  this->LabelMapIconLabel->hide();
  this->LabelMapOpacitySlider->hide();
  this->LabelMapOutlineButton->hide();
  this->LabelMapVisibilityButton->hide();
  this->SegmentSelectorWidget->hide();
  this->SegmentationIconLabel->hide();
  this->SegmentationOpacitySlider->hide();
  this->SegmentationOutlineButton->hide();
  this->SegmentationVisibilityButton->hide();
  this->BackgroundComboBox->hide();
  this->BackgroundIconLabel->hide();
  this->BackgroundInterpolationButton->hide();
  this->BackgroundOpacitySlider->hide();
  this->ForegroundComboBox->hide();
  this->ForegroundIconLabel->hide();
  this->ForegroundInterpolationButton->hide();
  this->ForegroundOpacitySlider->hide();
  this->LabelMapComboBox->hide();
  this->RulerButton->hide();
  //this->SliceOrientationSelector->setCurrentIndex(0);
...
void qMRMLSliceControllerWidgetPrivate::setAndObserveCrosshairNode()
{
  ...
  this->qvtkReconnect(nullptr, crosshairNode, vtkMRMLCrosshairNode::CursorShapeCenterEvent,
                      q, SLOT(updateCursorCenter()));
  this->qvtkReconnect(nullptr, crosshairNode, vtkMRMLCrosshairNode::CursorShapeHorEvent,
                      q, SLOT(updateCursorHor()));
  this->qvtkReconnect(nullptr, crosshairNode, vtkMRMLCrosshairNode::CursorShapeVerEvent,
                      q, SLOT(updateCursorVer()));
  this->qvtkReconnect(nullptr, crosshairNode, vtkMRMLCrosshairNode::CursorShapeSWEvent,
                      q, SLOT(updateCursorSW()));
  this->qvtkReconnect(nullptr, crosshairNode, vtkMRMLCrosshairNode::CursorShapeNWEvent,
                      q, SLOT(updateCursorNW()));
  this->qvtkReconnect(nullptr, crosshairNode, vtkMRMLCrosshairNode::CursorShapeRestoreEvent,
                      q, SLOT(updateCursorRestore()));
}
// --------------------------------------------------------------------------
...
void qMRMLSliceControllerWidgetPrivate::setupSliceModelMenu()
{
  ...
  this->SliceVisibilityButton->setArrowType(Qt::NoArrow);
...
//  this->SliceModelMenu->addMenu(dimensionsSliceModelMenu);
  ...
  fovSliceModelMenu->hide();
  dimensionsSliceModelMenu->hide();
  originSliceModelMenu->hide();
...
void qMRMLSliceControllerWidgetPrivate::setupMoreOptionsMenu()
{
  ...
  advancedMenu->addAction(this->actionReset_to_volume_plane);
    ...
    vtkMRMLSliceNode* sliceNode, QComboBox* sliceOrientationSelector)
{
  ...
  QSettings settings;
  QString strSettingResult = settings.value("IndustryType").toString();
  
  if(strSettingResult != "Medical"){
    sliceOrientationSelector->addItem("Reformat");
  } 
...
void qMRMLSliceControllerWidgetPrivate::updateWidgetFromMRMLSliceNode()
{
  ...
  // QMessageBox msgBox;
  // msgBox.setText( this->MRMLSliceNode->GetLayoutLabel());
  // msgBox.exec();
  QString strLabel = QString(this->MRMLSliceNode->GetLayoutLabel());
  QString strLabelText = "";
  if( strLabel == "R" )
  {
    strLabelText = tr("体軸断面 (axial)");
    this->SliceLabel->setStyleSheet("QLabel { color : red; }");
  } else if ( strLabel == "Y")
  {
    strLabelText = tr("矢状断面 (sagittal)"); 
    this->SliceLabel->setStyleSheet("QLabel { color : #f2b33d; }");
  }  else  {
    strLabelText = tr("冠状断面 (coronal)");
    this->SliceLabel->setStyleSheet("QLabel { color : green; }");
  } 
  
  this->SliceLabel->setText(strLabelText);
  ...
  this->SliceOffsetSlider->setStyleSheet("font-size: 10px;");
...
//                                                vtkMRMLSliceNode::SliceFOVMatch2DViewSpacingMatchVolumes);
  ...
  this->actionSliceModelModeVolumes_2D->setChecked(true);

  this->actionSliceModelModeVolumes_2D->trigger();
  double UVWExtents[] = {256,256,256};
  double UVWOrigin[] = {0,0,0};
  int UVWDimensions[] = {256,256,256};
...
void qMRMLSliceControllerWidgetPrivate::updateWidgetFromMRMLSliceCompositeNode()
{
  ...
  this->actionHotLinked->setChecked(true);
...
//  rulerTypesActions->addAction(this->actionRulerTypeNone);
  ...
  this->RulerTypesMapper->mapped(1);
  this->RulerColorMapper->mapped(1);

  this->OrientationMarkerTypesMapper->mapped(1);
  this->OrientationMarkerSizesMapper->mapped(1);
}
...
//---------------------------------------------------------------------------
void qMRMLSliceControllerWidget::resetSliceToBackground()
{
  Q_D(qMRMLSliceControllerWidget);
  if (!d->MRMLSliceNode)
    {
  ...
  QString strLabel = QString(d->MRMLSliceNode->GetLayoutLabel());
  if( strLabel == "R" )
  {
    d->SliceOrientationSelector->setCurrentIndex(1);
    QSettings settings;
    QString strSettingResult = settings.value("IndustryType").toString();

    if(strSettingResult == "Medical"){
      d->SliceOrientationSelector->setCurrentIndex(0);
    } else {
      d->SliceOrientationSelector->setCurrentIndex(3);
    }
  } else if ( strLabel == "Y")
  {
    d->SliceOrientationSelector->setCurrentIndex(0);
    ...
    d->SliceOrientationSelector->setCurrentIndex(1);
    d->SliceOrientationSelector->setCurrentIndex(2);
  } 
  // vtkSmartPointer<vtkCollection> nodes = d->saveNodesForUndo("vtkMRMLSliceNode");
  // if(!nodes.GetPointer())
  //   {
...
void qMRMLSliceControllerWidget::updateSegmentationControlsVisibility()
{
  ...
  bool popupVisible = true;//d->MoreButton->isChecked();
...
//------------------------------------------------------------------------------
void qMRMLSliceControllerWidget::updateCursorCenter()
{
  Q_D(qMRMLSliceControllerWidget);
...
//------------------------------------------------------------------------------
void qMRMLSliceControllerWidget::updateCursorHor()
{
  Q_D(qMRMLSliceControllerWidget);
...
//------------------------------------------------------------------------------
void qMRMLSliceControllerWidget::updateCursorVer()
{
  Q_D(qMRMLSliceControllerWidget);
...
//------------------------------------------------------------------------------
void qMRMLSliceControllerWidget::updateCursorSW()
{
  Q_D(qMRMLSliceControllerWidget);
...
//------------------------------------------------------------------------------
void qMRMLSliceControllerWidget::updateCursorNW()
{
  Q_D(qMRMLSliceControllerWidget);
...
//------------------------------------------------------------------------------
void qMRMLSliceControllerWidget::updateCursorRestore()
{
  Q_D(qMRMLSliceControllerWidget);
