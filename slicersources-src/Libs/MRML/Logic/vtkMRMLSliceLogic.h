...
public:
  ...
  ///
  /// \brief GetVolumeSliceBounds returns the bounds for a slice orientation in a volume
  /// \param volumeNode the volume of the slice node
  /// \param sliceNode the slice with set orientation
  /// \param sliceBounds the output bounds or 0,0,0,0,0,0 if no input slice node.
  /// \sa GetVolumeSliceBounds()
  static void GetVolumeSliceBounds(vtkMRMLVolumeNode *volumeNode, vtkMRMLSliceNode* sliceNode, double sliceBounds[6]);
