...
void vtkMRMLSliceLayerLogic::UpdateVolumeDisplayNode()
{
  ...
  this->UpdateVolumeDisplayFromObservedNode(this->VolumeDisplayNode,
                                            this->VolumeDisplayNodeObserved);
  this->UpdateVolumeDisplayFromObservedNode(this->VolumeDisplayNodeUVW,
                                            this->VolumeDisplayNodeObserved);
}

//----------------------------------------------------------------------------
void vtkMRMLSliceLayerLogic::UpdateVolumeDisplayFromObservedNode(
    vtkMRMLVolumeDisplayNode* volumeDisplayNode,
    vtkMRMLVolumeDisplayNode* volumeDisplayNodeObserved)
{
  if (volumeDisplayNode == nullptr ||
      volumeDisplayNodeObserved == nullptr)
    {
    return;
    }

  int wasDisabling = volumeDisplayNode->GetDisableModifiedEvent();
  volumeDisplayNode->SetDisableModifiedEvent(1);
  // copy the scene first because Copy() might need the scene
  volumeDisplayNode->SetScene(volumeDisplayNodeObserved->GetScene());
  volumeDisplayNode->Copy(volumeDisplayNodeObserved);
  ...
  if (scalarVolumeDisplayNode)
    ...
    scalarVolumeDisplayNode->SetAutoWindowLevel(0);
    scalarVolumeDisplayNode->SetAutoThreshold(0);
    if (this->SliceNode
        && this->SliceNode->GetSlabMode() == VTK_IMAGE_SLAB_SUM
        && this->SliceNode->GetSlabNumberOfSlices() != 1)
      {
      int sliceNumber = this->GetSlabNumberOfSlices();
      scalarVolumeDisplayNode->SetWindowLevel(
            scalarVolumeDisplayNode->GetWindow() * sliceNumber,
            scalarVolumeDisplayNode->GetLevel() * sliceNumber);
      }
    }
  volumeDisplayNode->SetDisableModifiedEvent(wasDisabling);
}
...
void vtkMRMLSliceLayerLogic::UpdateTransforms()
{
  ...
  ***/
  ...
  if (this->SliceNode)
    {
    this->Reslice->SetSlabNumberOfSlices( this->GetSlabNumberOfSlices() );
    // If slab mode is sum and number of slices is != 1, then we need to cast
    // the output in an appropriate format.
    this->Reslice->SetSlabMode(this->SliceNode->GetSlabMode());
    }
  this->UpdatingTransforms = 0;
...
//----------------------------------------------------------------------------
int vtkMRMLSliceLayerLogic::GetSlabNumberOfSlices()
{
  int slabNumberOfSlices = this->SliceNode->GetSlabNumberOfSlices();
  if (slabNumberOfSlices < 1)
    {
    double bounds[6];
    vtkMRMLSliceLogic::GetVolumeSliceBounds(this->VolumeNode, this->SliceNode, bounds);
    double normal[3];
    this->SliceNode->GetSliceNormal(normal);
    normal[0] *= this->SliceNode->GetPrescribedSliceSpacing()[0];
    normal[1] *= this->SliceNode->GetPrescribedSliceSpacing()[1];
    normal[2] *= this->SliceNode->GetPrescribedSliceSpacing()[2];
    slabNumberOfSlices = static_cast<int>(
          (bounds[5] - bounds[4]) / vtkMath::Norm(normal));
    }
  return slabNumberOfSlices;
}
