...
class VTK_MRML_EXPORT vtkMRMLSliceNode : public vtkMRMLAbstractViewNode
{
  ...
  ///
  /// Return the normal (x,y,z) to the slice plane.
  /// It is the last column of the SliceToRAS matrix.
  /// \sa GetSliceToRAS(), GetSliceSpacing()
  void GetSliceNormal(double normal[3]);

  ///
  /// \brief Return the slice spacing
  /// \return the space between 2 slices
  /// \sa GetSliceNormal()
  ...
  //bool SetOrientation1(const char* orientation);
...
public:
  ...
  void JumpSlice(double r, double a, double s, bool moveX, bool moveY);
  int GetNodeIndex();
  void SetMipThickness(int thickness);
  int GetMipThickness();
  ...
  ///
  /// \brief Set the number of slab slices
  /// \sa SlabNumberOfSlices
  vtkSetMacro(SlabNumberOfSlices, int);
  ///
  /// \brief Get the number of slab slices
  /// \sa SlabNumberOfSlices
  vtkGetMacro(SlabNumberOfSlices, int);

  ///
  /// \brief Set the slab mode
  /// \sa SlabMode
  vtkSetMacro(SlabMode, int);
  ///
  /// \brief Get the slab mode
  /// \sa SlabMode
...
protected:
  ...
  ///
  /// \brief This property controls the number of slices that will be combined
  /// to create a slab.
  /// 1 (default) means no slab, <1 means all the slices.
  /// \sa GetSlabNumberOfSlices(), SetSlabNumberOfSlices(), SlabMode
  int SlabNumberOfSlices;
  ///
  /// \brief This property controls the slab mode. Average by default.
  /// See vtkImageReslice::SlabMode for the different values
  /// \sa GetSlabMode(), SetSlabMode(), SlabNumberOfSlices
  int SlabMode;

  /// \sa GetMipThickness(), SetMipThickness()
  int MipThickness;
