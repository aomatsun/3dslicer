...
vtkMRMLScalarVolumeDisplayNode::vtkMRMLScalarVolumeDisplayNode()
{
  ...
  this->ApplyStencil = vtkImageStencil::New();
  ...
  this->ApplyStencil->SetInputConnection(0, this->Threshold->GetOutputPort());

  this->AppendComponents->RemoveAllInputs();
  this->AppendComponents->AddInputConnection(0, this->ExtractRGB->GetOutputPort() );
  this->AppendComponents->AddInputConnection(0, this->ApplyStencil->GetOutputPort() );
...
{
  this->ApplyStencil->SetStencilConnection(imageDataConnection);
}
//----------------------------------------------------------------------------
vtkAlgorithmOutput* vtkMRMLScalarVolumeDisplayNode::GetBackgroundImageStencilDataConnection()
{
  ...
  const int stencilInputPort = 1;
  return this->ApplyStencil->GetNumberOfInputConnections(0) > stencilInputPort ?
         this->ApplyStencil->GetInputConnection(0,stencilInputPort) : nullptr;
}
