...
void qSlicerCoreApplicationPrivate::init()
{
  ...
  // Set standard library local to "C" only for numbers
  // Using LC_ALL will mangle system path when converting QStrings to/from const char*
  // for paths containing non "C" characters such as japanese characters.
  setlocale(LC_ALL, qPrintable(QLocale::system().name()));
...
//----------------------------------------------------------------------------
inline QStringList findTranslationFilesWithLanguageExtension(const QString& dir, const QString& languageExtension) 
{
...
//----------------------------------------------------------------------------
inline QStringList findTranslationFiles(const QString& dir, const QString& settingsLanguage) 
{
  ...
  auto files = findTranslationFilesWithLanguageExtension(dir, settingsLanguage);
  if (files.isEmpty())
    {
    // If no specific translations have been found, look for a generic language extension
    // (In case of fr_FR -> look for *fr.qm files).
    const auto genericExtension = settingsLanguage.split("_")[0];
...
void qSlicerCoreApplication::loadTranslations(const QString& dir)
{
  ...
  QStringList qmFiles = findTranslationFiles(dir, app->settings()->value("language").toString());
