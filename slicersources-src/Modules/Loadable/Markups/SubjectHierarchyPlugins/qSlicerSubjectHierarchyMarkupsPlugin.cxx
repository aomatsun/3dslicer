...
void qSlicerSubjectHierarchyMarkupsPluginPrivate::init()
{
  ...
  this->ToggleHandleInteractive = new QAction("表示/非表示");
  ...
  this->RenamePointAction->setVisible(false);
  this->DeletePointAction->setVisible(false);
  this->ToggleSelectPointAction->setVisible(false);
  // this->ToggleHandleInteractive->setVisible(false);
...
void qSlicerSubjectHierarchyMarkupsPlugin::setDisplayColor(vtkIdType itemID, QColor color, QMap<int, QVariant> terminologyMetaData)
{
  ...
  if (oldColor != color)
    ...
    QString markerName = QString(displayNode->GetName());
    //  MarkupsFiducialDisplay_1 or MarkupsDisplay_1 => ModelDisplay_1
    // markerName.replace(0, QString("MarkupsFiducialDisplay").length(), QString("ModelDisplay"));
    markerName.replace(0, QString("MarkupsDisplay").length(), QString("ModelDisplay"));
    // outputLogInt(markerName.toLocal8Bit().data(), 300);

    vtkMRMLScene* scene = displayNode->GetScene();
    if (scene)
    {
        int nnodes = scene->GetNumberOfNodesByClass("vtkMRMLDisplayNode");
        int n;
        for (n = 0; n < nnodes; n++)
        {
            vtkMRMLDisplayNode* node = vtkMRMLDisplayNode::SafeDownCast(
                scene->GetNthNodeByClass(n, "vtkMRMLDisplayNode"));
            if (node == nullptr || QString(node->GetName()) != markerName)
                continue;

            double* colors = node->GetSelectedColor();
            // outputLogDouble("--- Fiducial color: ", colors);
            node->SetColor(color.redF(), color.greenF(), color.blueF());
            break;
        }
    }
...
void qSlicerSubjectHierarchyMarkupsPlugin::showViewContextMenuActionsForItem(vtkIdType itemID, QVariantMap eventData)
{
  ...
  if (associatedNode)
    ...
    d->RenamePointAction->setVisible(false);
    d->DeletePointAction->setVisible(false);
    d->ToggleSelectPointAction->setVisible(false);
  //  d->ToggleHandleInteractive->setVisible(false);
