...
QColor qSlicerSubjectHierarchyModelsPlugin::getDisplayColor(vtkIdType itemID, QMap<int, QVariant> &terminologyMetaData)const
{
  ...
  if (scene)
  {
    ...
    //  ModelDisplay_1 => MarkupsFiducialDisplay_1 or MarkupsDisplay_1
    // markerName.replace(0, QString("ModelDisplay").length(), QString("MarkupsFiducialDisplay"));
    markerName.replace(0, QString("ModelDisplay").length(), QString("MarkupsDisplay"));
    // outputLogInt(markerName.toLocal8Bit().data(), 250);
    ...
    for (n = 0; n < nnodes; n++)
    {
      ...
      colorArray = node->GetSelectedColor();
      // outputLogDouble("--- Fiducial color: ", colorArray);
      displayNode->SetColor(colorArray[0], colorArray[1], colorArray[2]);
      break;
      }
  }
