...
class SegmentEditorIslandsEffect(AbstractScriptedSegmentEditorEffect):
  ...
  def helpText(self):
    return self.tr("Edit islands (connected components) in a segment.")

  def setupOptionsFrame(self):
    self.operationRadioButtons = []

    self.keepLargestOptionRadioButton = qt.QRadioButton(self.tr("Keep largest island"))
    self.keepLargestOptionRadioButton.setToolTip(
      self.tr("Keep only the largest island in selected segment, remove all other islands in the segment."))
    ...
    self.keepSelectedOptionRadioButton = qt.QRadioButton(self.tr("Keep selected island"))
    self.keepSelectedOptionRadioButton.setToolTip(self.tr(
    ...
    self.removeSmallOptionRadioButton = qt.QRadioButton(self.tr("Remove small islands"))
    self.removeSmallOptionRadioButton.setToolTip(
      self.tr("Remove all islands from the selected segment that are smaller than the specified minimum size."))
    ...
    self.removeSelectedOptionRadioButton = qt.QRadioButton(self.tr("Remove selected island"))
    self.removeSelectedOptionRadioButton.setToolTip(
      self.tr("Click on an island to remove it from selected segment."))
    ...
    self.addSelectedOptionRadioButton = qt.QRadioButton(self.tr("Add selected island"))
    self.addSelectedOptionRadioButton.setToolTip(
      self.tr("Click on a region to add it to selected segment."))
    ...
    self.splitAllOptionRadioButton = qt.QRadioButton(self.tr("Split islands to segments"))
    self.splitAllOptionRadioButton.setToolTip(self.tr(
      "Create a new segment for each island of selected segment. Islands smaller than minimum size will be removed. ") +
      self.tr("Segments will be ordered by island size."))
    ...
    self.minimumSizeSpinBox.setToolTip(self.tr(
    ...
    self.minimumSizeSpinBox.suffix = self.tr(" voxels")
    self.minimumSizeLabel = self.scriptedEffect.addLabeledOptionsWidget(self.tr("Minimum size:"),
                                                                        self.minimumSizeSpinBox)

    self.applyButton = qt.QPushButton(self.tr("Apply"))
  ...
  def updateGUIFromMRML(self):
    ...
    if segmentSelectionRequired:
      self.applyButton.setToolTip(self.tr("Click in a slice viewer to select segment"))
    else:
