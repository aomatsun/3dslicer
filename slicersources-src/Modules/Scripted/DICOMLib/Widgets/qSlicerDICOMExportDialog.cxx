...
void qSlicerDICOMExportDialogPrivate::init()
{
  ...
  // Use configured output folder if any, else set Slicer DICOM database folder as default output folder
  QString outputDirPath = this->OutputDirPath;
  if (outputDirPath.isEmpty() && qSlicerApplication::application()->dicomDatabase())
    {
      outputDirPath = qSlicerApplication::application()->dicomDatabase()->databaseDirectory();
    }
  this->DirectoryButton_OutputFolder->setDirectory(outputDirPath);
  ...
  // this->SubjectHierarchyTreeView->hide();
  // this->ExportablesListWidget->hide();
  // this->DICOMTagEditorWidget->hide();
  // this->ExportSeriesRadioButton->hide();
  // this->SaveTagsCheckBox->hide();
  // this->ImportExportedDatasetCheckBox->hide();
...
//-----------------------------------------------------------------------------
void qSlicerDICOMExportDialog::setOutputDirectory(const QString& dirPath)
{
    Q_D(qSlicerDICOMExportDialog);
    d->OutputDirPath = dirPath;
}
...
void qSlicerDICOMExportDialog::exportSeries()
{
  ...
  if (d->DICOMTagEditorWidget->exportables().isEmpty())
    ...
    QPixmap  pixmap = QPixmap("Cursor.png");
    QCursor cursor = QCursor(pixmap, 32, 32);
