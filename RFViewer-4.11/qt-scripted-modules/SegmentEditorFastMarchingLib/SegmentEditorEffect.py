...
class SegmentEditorEffect(AbstractScriptedSegmentEditorEffect):
  ...
  def helpText(self):
    return self.tr("""<html>Expand the selected segment<br> to regions that have similar intensity.<p>
...
<p></html>""")
  ...
  def setupOptionsFrame(self):
    ...
    self.percentMax.setToolTip(self.tr('Approximate volume of the structure to be segmented as percentage of total volume of the master image.'
    ...
    self.march = qt.QPushButton(self.tr("Initialize"))
    self.march.setToolTip(self.tr("Perform the Marching operation into the current label map"))
    ...
    self.percentVolume = self.scriptedEffect.addLabeledOptionsWidget(self.tr("Segment volume:"), self.marcher)
    
    self.cancelButton = qt.QPushButton(self.tr("Cancel"))
    self.cancelButton.objectName = self.__class__.__name__ + 'Cancel'
    self.cancelButton.setToolTip(self.tr("Clear preview and cancel"))

    self.applyButton = qt.QPushButton(self.tr("Apply"))
    self.applyButton.objectName = self.__class__.__name__ + 'Apply'
    self.applyButton.setToolTip(self.tr("Replace segment by previewed result"))
  ...
  def onMarch(self):
    ...
    try:
      ...
      slicer.util.showStatusMessage(self.tr('Running FastMarching...'), 2000)
      self.scriptedEffect.saveStateForUndo()
      self.fastMarching(self.percentMax.value)
      slicer.util.showStatusMessage(self.tr('FastMarching finished'), 2000)
