...
import logging
from RFViewerHomeLib import ExportDirectorySettings, warningMessageBox
...
class ScreenCaptureWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    self.inputCollapsibleButton.text = "キャプチャー範囲"
    self.inputCollapsibleButton.setStyleSheet("color:black; background-color:white;")
    ...
    self.captureAllViewsCheckBox = qt.QRadioButton(" ")
    self.captureAllViewsCheckBox.checked = True
    ...
    inputFormLayout.addRow("全体表示:", self.captureAllViewsCheckBox)
    self.captureAllViewsCheckBox.setStyleSheet("color:black; background-color:white;")

    self.capture3DViewsCheckBox = qt.QRadioButton(" ")
    self.capture3DViewsCheckBox.checked = False
    self.capture3DViewsCheckBox.setToolTip("If checked, 3D views will be captured. If unchecked then only the selected view will be captured.")
    inputFormLayout.addRow("3Ｄ表示:", self.capture3DViewsCheckBox)
    self.capture3DViewsCheckBox.setStyleSheet("color:black; background-color:white;")

    self.captureRViewsCheckBox = qt.QRadioButton(" ")
    self.captureRViewsCheckBox.checked = False
    self.captureRViewsCheckBox.setToolTip("If checked, R views will be captured. If unchecked then only the selected view will be captured.")
    inputFormLayout.addRow("体軸断面(axial):", self.captureRViewsCheckBox)
    self.captureRViewsCheckBox.setStyleSheet("color:black; background-color:white;")

    self.captureYViewsCheckBox = qt.QRadioButton(" ")
    self.captureYViewsCheckBox.checked = False
    self.captureYViewsCheckBox.setToolTip("If checked, Y views will be captured. If unchecked then only the selected view will be captured.")
    inputFormLayout.addRow("矢状断面（sagittal）:", self.captureYViewsCheckBox)
    self.captureYViewsCheckBox.setStyleSheet("color:black; background-color:white;")

    self.captureGViewsCheckBox = qt.QRadioButton(" ")
    self.captureGViewsCheckBox.checked = False
    self.captureGViewsCheckBox.setToolTip("If checked, G views will be captured. If unchecked then only the selected view will be captured.")
    inputFormLayout.addRow("冠状断面（coronal）:", self.captureGViewsCheckBox)
    self.captureGViewsCheckBox.setStyleSheet("color:black; background-color:white;")
    ...
    self.sliceStartOffsetSliderWidget.singleStep = 1
  ...
  #  outputFormLayout = qt.QFormLayout(self.outputCollapsibleButton)
   ...
   # outputFormLayout.addRow("Output type:", self.outputTypeWidget)
    ...
    self.numberOfStepsSliderWidget.value = 1
...
#    outputFormLayout.addRow("Output file name:", hbox)
 ...
 #   advancedFormLayout.addRow("ffmpeg executable:", self.ffmpegPathSelector)
  ...
  #  advancedFormLayout.addRow("", self.videoExportFfmpegWarning)
   ...
   # advancedFormLayout.addRow("Video extra options:", self.extraVideoOptionsWidget)
    ...
    # self.fileNamePatternWidget.text = "Session_" + datetime.now()
    self.fileNamePatternWidget.text = "Session_" + strnow + "_%05d.jpg"
  ...
  #  advancedFormLayout.addRow("", hbox)
    ...
    # Capture button
    self.captureButtonLabelCapture = "保存"
 ...
 #   self.layout.addWidget(self.statusLabel)
  ...
  def updateOutputType(self, selectionIndex=0):
    ...
    self.fileNamePatternWidget.enabled = False
    self.videoFileNameWidget.enabled = False
    self.videoFormatWidget.enabled = False
    self.videoLengthSliderWidget.enabled = False
    self.videoFrameRateSliderWidget.enabled = False
    self.videoFileNameWidget.setVisible(False)
    self.videoFileNameWidget.enabled = False
    self.lightboxImageFileNameWidget.setVisible(False)
    self.lightboxImageFileNameWidget.enabled = False
...
#    self.sequenceBrowserNodeSelectorWidget.visible = (self.animationMode == "sequence")
 ...
 #   self.sequenceEndItemIndexWidget.visible = (self.animationMode == "sequence")
  ...
  def onCaptureButton(self):
    ...
    if self.capture3DViewsCheckBox.checked == True: 
      self.viewNodeSelector.setCurrentNodeIndex(0)
    elif self.captureRViewsCheckBox.checked == True:
      self.viewNodeSelector.setCurrentNodeIndex(1)
    elif self.captureYViewsCheckBox.checked == True:
      self.viewNodeSelector.setCurrentNodeIndex(2)
    elif self.captureGViewsCheckBox.checked == True:
      self.viewNodeSelector.setCurrentNodeIndex(3)
   ...
   # self.enableInputOutputWidgets(False)
    ...
    outputDir = ExportDirectorySettings.load()
    ...
    try:
      ...
      # warningMessageBox(self.tr("保存"), self.tr("保存されました"))
